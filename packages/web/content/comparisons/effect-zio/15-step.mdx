...
<CrossLanguageBlock
  ...
  effectCode={`
    ...
    |  )
  `}
  zioComment="jdbc.query(sql\"...\").map row parser"
  effectComment="sql\`...\` template literal, automatic parameterization"
/>
...
<CrossLanguageBlock
  ...
  effectCode={`
    ...
    |const transfer: Effect<void, SqlClient.SqlError, SqlClient> =
    |  Effect.gen(function* () {
...
<CrossLanguageBlock
  ...
  effectComment="sql\`INSERT/UPDATE ...\` returns Effect<void>"
/>
...
<CrossLanguageBlock
  ...
  effectCode={{
    ...
    |})
    |
    |const getUsers: Effect<
    |  ReadonlyArray<User>,
    |  SqlClient.SqlError | Schema.Decode.Error,
    |  SqlClient
    |> = Effect.flatMap(SqlClient, (sql) =>
    |    Effect.map(
    |      sql\`SELECT * FROM users\`,
...
|----------|--------|---------|
| `jdbc.query(sql\"...\")` | `sql\`...\`` | Execute query |
| `jdbc.update(sql\"...\")` | `sql\`INSERT/UPDATE ...\`` | Execute update |
...
<Callout variant="tip">
Use template literal sql\`...\` for SQL queries in Effect. This automatically parameterizes your queries, preventing SQL injection. The template variables become bound parameters.
</Callout>
