...
- [x] Completed: **Audit zio-cats steps 11-15 against Zionomicon** — Audited steps 11-15 for content structure, validate=false usage, and external library handling. All 5 steps have substantial content (308-612 lines each) with proper `validate={false}` on all snippets using external libraries. Step 11 (STM) uses cats-stm (coop), Step 12 uses cats-effect-std and fs2, Step 13 uses ciris, Step 14 uses zio-http/http4s, Step 15 uses zio-jdbc/doobie. All ScalaComparisonBlock code uses correct `|` prefix format. Glossary already covers STM (6), CONFIG (7), HTTP (7), DATABASE (6) entries. Files: `/Users/hmemcpy/git/toolkata-content/zio-cats/lessons/11-step.mdx` through `15-step.mdx`, `packages/web/content/glossary/zio-cats.ts`

- [ ] **Update zio-cats steps 11-15 per audit findings** — Audit found content structure is correct with proper validate=false usage and | prefix format. Remaining work is accuracy verification against Zionomicon ePub (not accessible in build mode). Files: `/Users/hmemcpy/git/toolkata-content/zio-cats/lessons/11-step.mdx` through `15-step.mdx`

- [ ] **Update zio-cats glossary if needed** — Verified glossary has 44 entries across 11 categories: STM (6 entries), CONFIG (7), HTTP (7), DATABASE (6), CONCURRENCY (7), BASICS (7), ERRORS (5), DEPENDENCIES (5), STREAMING (7), RUNTIME (5), INTEROP (4). All advanced topics from steps 11-15 are covered. Remaining work is accuracy verification against Zionomicon. File: `packages/web/content/glossary/zio-cats.ts`
